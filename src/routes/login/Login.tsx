...
export function Login() {
  ...
  const handleLogin = async (res: any) => {
    if (!res.profileObj)
      return setError({ error: "Failed to fetch Google user" });
    let user_config = await userRequests
      .GET(res.profileObj.googleId)
      .catch((err: any) => console.log({ err }));
    if (!user_config) {
      const response = await userRequests.ADD({
        id: res.profileObj.googleId,
        name: res.profileObj.name,
        email: res.profileObj.email,
      });
      if (!response.ok) return setError(response);
      user_config = await userRequests
        .GET(res.profileObj.googleId)
        .catch((err: any) => console.log({ err }));
    }
